...
def scatter_plot(path, x, y):
    ...
    Save to html file.
    Input:
    "data_for_lr.csv" must exist in the path input.
    x, y must be string. Choose continuous variables from {IMDB.Rating,
    IMDB.Votes, Language,Runtime, budget, actor_popularity,
    director_popularity, revenue}
    '''
    ...
    try:
        ...
        return "Invalid Input. Check Docstring."
...
def box_plot(path, x):
    ...
    Input:
    "data_for_lr.csv" must exist in the path input.
    x must be string. Choose binary variables from {released_on_weekend,
    released_not_on_dump_month, genre[Action, Adventure, Animation...], G,
    R, PG, PG-13}
    '''
    df = pd.read_csv(path + "/data_for_lr.csv")
